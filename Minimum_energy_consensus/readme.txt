# Minimum Time-Energy Consensus for Multi-Agent Systems

## 📖 Overview
This repository contains a comprehensive MATLAB framework for analyzing and simulating minimum time and energy-bounded consensus protocols for multi-agent systems. The codebase provides algorithms for computing optimal consensus times, defining energy attainable sets (via ellipsoidal approximations), and visualizing state trajectories and control inputs across various system dynamics (e.g., double integrators, third-order systems).
